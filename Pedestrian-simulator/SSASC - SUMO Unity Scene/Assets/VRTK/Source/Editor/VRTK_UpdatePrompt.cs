...
#pragma warning restore 649
    ...
    private static UnityWebRequest versionResource;
    ...
    private static void CheckForUpdate()
    {
        ...
        versionResource = versionResource == null ? StartRequest(remoteURL) : versionResource;
        ...
        if (UrlSuccess(versionResource))
        {
            latestRelease = LatestRelease.CreateFromJSON(versionResource.downloadHandler.text);
        }
    ...
    private static UnityWebRequest StartRequest(string url)
    {
    ...
    private static bool UrlSuccess(UnityWebRequest request)
    {
        if (request == null)
        {
            return false;
        ...
        return request.downloadHandler != null && !string.IsNullOrEmpty(request.downloadHandler.text);
    }

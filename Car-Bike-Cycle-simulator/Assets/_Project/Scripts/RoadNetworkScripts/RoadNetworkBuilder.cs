...
public class RoadNetworkBuilder : MonoBehaviour
{
    ...
    public void LoadSumoXmlFiles(string sumoFilesFolder)
    {
        ...
        string polyFilePath = ResolvePolygonFilePath(sumoXmlFolderPath);
        ...
        if (string.IsNullOrEmpty(polyFilePath) || !File.Exists(polyFilePath))
        {
            Debug.LogWarning("No polygon file found. Terrain/land polygons will not be generated.");
            return;
        }
    ...
    private static string ResolvePolygonFilePath(string folderPath)
    {
        string[] candidateNames =
        {
            "Sumo2Unity.poly.xml",
            "Sumo2Unity.Poly.xml",
            "Sumo2Unity.poly.xml.add.xml",
            "Sumo2Unity.Poly.xml.add.xml"
        };

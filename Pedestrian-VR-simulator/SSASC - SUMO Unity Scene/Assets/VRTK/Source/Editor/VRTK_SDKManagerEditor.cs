namespace VRTK
{
    ...
    public class VRTK_SDKManagerEditor : Editor
    {
            ...
            using (new EditorGUILayout.VerticalScope("Box"))
            {
                ...
                VRTK_EditorUtilities.DrawUsingDestructiveStyle(GUI.skin.button, style =>
                {
                    ...
                    if (GUILayout.Button(clearSymbolsGUIContent, style))
                    {
                        ...
                        foreach (BuildTargetGroup targetGroup in targetGroups)
                        {
                            string[] currentSymbols = GetScriptingDefineSymbolsCompat(targetGroup)
                                .Split(';')
                                .Distinct()
                            ...
                            SetScriptingDefineSymbolsCompat(targetGroup, string.Join(";", newSymbols));
        ...
        private static string GetScriptingDefineSymbolsCompat(BuildTargetGroup targetGroup)
        {
            Type namedBuildTargetType = Type.GetType("UnityEditor.NamedBuildTarget, UnityEditor");
            if (namedBuildTargetType != null)
            {
                MethodInfo fromBuildTargetGroupMethod = namedBuildTargetType.GetMethod("FromBuildTargetGroup", BindingFlags.Public | BindingFlags.Static);
                MethodInfo getSymbolsMethod = typeof(PlayerSettings).GetMethod("GetScriptingDefineSymbols", BindingFlags.Public | BindingFlags.Static, null, new[] { namedBuildTargetType }, null);
                if (fromBuildTargetGroupMethod != null && getSymbolsMethod != null)
                {
                    object namedBuildTarget = fromBuildTargetGroupMethod.Invoke(null, new object[] { targetGroup });
                    return (string)getSymbolsMethod.Invoke(null, new[] { namedBuildTarget });
                }
            }
        ...
        private static void SetScriptingDefineSymbolsCompat(BuildTargetGroup targetGroup, string symbols)
        {
            Type namedBuildTargetType = Type.GetType("UnityEditor.NamedBuildTarget, UnityEditor");
            if (namedBuildTargetType != null)
            {
                MethodInfo fromBuildTargetGroupMethod = namedBuildTargetType.GetMethod("FromBuildTargetGroup", BindingFlags.Public | BindingFlags.Static);
                MethodInfo setSymbolsMethod = typeof(PlayerSettings).GetMethod("SetScriptingDefineSymbols", BindingFlags.Public | BindingFlags.Static, null, new[] { namedBuildTargetType, typeof(string) }, null);
                if (fromBuildTargetGroupMethod != null && setSymbolsMethod != null)
                {
                    object namedBuildTarget = fromBuildTargetGroupMethod.Invoke(null, new object[] { targetGroup });
                    setSymbolsMethod.Invoke(null, new[] { namedBuildTarget, symbols });
                    return;
                }
            }

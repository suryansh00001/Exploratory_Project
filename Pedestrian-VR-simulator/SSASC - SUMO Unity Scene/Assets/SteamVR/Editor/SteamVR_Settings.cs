...
public class SteamVR_Settings : EditorWindow
{
	...
	static void Update()
	{
...
#else
			var devices = GetVREnabledDevicesCompat(BuildTargetGroup.Standalone);
#endif
			var hasOpenVR = false;
...
#else
				SetVREnabledDevicesCompat(BuildTargetGroup.Standalone, newDevices);
#endif
			}

			// Suppress repetitive info logs in newer Unity editor versions.
		}

		// Keep native OpenVR DLLs in Assets/Plugins for legacy SteamVR runtime support.
		// Deleting these files causes DllNotFoundException: openvr_api at play mode startup.

		EditorApplication.update -= Update;
	}

		static string[] GetVREnabledDevicesCompat(BuildTargetGroup targetGroup)
		{
			var vrEditorType = System.Type.GetType("UnityEditorInternal.VR.VREditor, UnityEditor");
			if (vrEditorType == null)
			{
				return new string[0];
			}

			var getDevices = vrEditorType.GetMethod("GetVREnabledDevices", new[] { typeof(BuildTargetGroup) });
			if (getDevices != null)
			{
				return (string[])getDevices.Invoke(null, new object[] { targetGroup });
			}

			var getDevicesOnTargetGroup = vrEditorType.GetMethod("GetVREnabledDevicesOnTargetGroup", new[] { typeof(BuildTargetGroup) });
		...
		static void SetVREnabledDevicesCompat(BuildTargetGroup targetGroup, string[] devices)
		{
			var vrEditorType = System.Type.GetType("UnityEditorInternal.VR.VREditor, UnityEditor");
			if (vrEditorType == null)
			{
			...
			var setDevices = vrEditorType.GetMethod("SetVREnabledDevices", new[] { typeof(BuildTargetGroup), typeof(string[]) });
			if (setDevices != null)
			{
				setDevices.Invoke(null, new object[] { targetGroup, devices });
				return;
			}

			var setDevicesOnTargetGroup = vrEditorType.GetMethod("SetVREnabledDevicesOnTargetGroup", new[] { typeof(BuildTargetGroup), typeof(string[]) });
			if (setDevicesOnTargetGroup != null)
			{
				setDevicesOnTargetGroup.Invoke(null, new object[] { targetGroup, devices });
			}
		}
